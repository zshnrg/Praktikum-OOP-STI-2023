...
public class PasswordManager {
    ...
    public static StringBuilder encrypt(String password, int shift) {
        ...
        for (int i = 0; i < pass.length(); i++) {
            pass.setCharAt(i, (char) (((int) pass.charAt(i) + shift - 'a' + 26) % 26 + 'a'));
        }
        return pass;
    }
    ...
    public static void addPassword(String password, int shift) {
        passwords.add(new String(encrypt(password, shift)));
        shifts.add(shift);
    }
    ...
    public static String retrievePassword(int indeks) {
        return new String(decrypt(passwords.get(indeks), shifts.get(indeks)));
    }
}